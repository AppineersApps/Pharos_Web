...
class Wscontroller extends Cit_Controller
{
    ...
    public function WSExecuter($func_arg = '', $id_arg = NULL)
    {
        ...
        $api_allowed_without_access_token = array('social_login','user_login_phone','user_login_email','send_verification_link','check_unique_user','social_sign_up','user_sign_up_phone','business_sign_up_email','user_sign_up_email','user_email_confirmation','static_pages','send_sms','reset_password_confirmation','reset_password_phone','get_template_message','forgot_password_phone','reset_password','forgot_password','states_list','get_config_paramaters','business_type_list','social_sign_up_business');
        ...
        if($appFailled == 'No' AND !in_array($func_name, $api_allowed_without_access_token))
        {
            ...
            if($req_token)
            {
                ...
                /* if(true== empty($userid)) {
                    $this->db->select('iBusinessId,eStatus');
                $this->db->from('business');
                $this->db->where('vAccessToken',$access);
                //$this->db->where('eStatus','Active');
                $result = $this->db->get()->result_array();
                $userid = $result[0]['iBusinessId']; 
                $status = $result[0]['eStatus'];  
