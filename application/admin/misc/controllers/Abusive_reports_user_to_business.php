...
class Abusive_reports_user_to_business extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('abusive_reports_user_to_business_model');
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "abusive_reports_user_to_business_list",
                    "abusive_reports_user_to_business_view",
                    "abusive_reports_user_to_business_add",
                    "abusive_reports_user_to_business_update",
                    "abusive_reports_user_to_business_delete",
                    "abusive_reports_user_to_business_export",
                    "abusive_reports_user_to_business_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "abusive_reports_user_to_business");
            $status_array = $status_label = array();
            $list_config = $this->abusive_reports_user_to_business_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "abusive_reports_user_to_business");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->abusive_reports_user_to_business_model->default_filters,
            );
            ...
            {
                $this->loadView("abusive_reports_user_to_business_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->abusive_reports_user_to_business_model->getListConfiguration();
        $form_config = $this->abusive_reports_user_to_business_model->getFormConfiguration();
        $extra_cond = $this->abusive_reports_user_to_business_model->extra_cond;
        $groupby_cond = $this->abusive_reports_user_to_business_model->groupby_cond;
        $having_cond = $this->abusive_reports_user_to_business_model->having_cond;
        $orderby_cond = $this->abusive_reports_user_to_business_model->orderby_cond;
        ...
        $data_recs = $this->abusive_reports_user_to_business_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_ABUSIVE_REPORTS_FOR_USER_DATA_FOUND_C46_C46_C33');

        $data_recs = $this->abusive_reports_user_to_business_model->processCustomLinks($data_recs, $list_config);
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("abusive_reports_user_to_business_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->abusive_reports_user_to_business_model->getListConfiguration();
        $form_config = $this->abusive_reports_user_to_business_model->getFormConfiguration();
        $table_name = $this->abusive_reports_user_to_business_model->table_name;
        $table_alias = $this->abusive_reports_user_to_business_modeltable_alias;
        $primary_key = $this->abusive_reports_user_to_business_model->primary_key;
        $extra_cond = $this->abusive_reports_user_to_business_model->extra_cond;
        $groupby_cond = $this->abusive_reports_user_to_business_model->groupby_cond;
        $having_cond = $this->abusive_reports_user_to_business_model->having_cond;
        $orderby_cond = $this->abusive_reports_user_to_business_model->orderby_cond;
        ...
        $db_recs = $this->abusive_reports_user_to_business_model->getExportData($export_config);
        ...
        $misc_info['filename'] = "abusive_reports_user_to_business_for_user_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("abusive_reports_user_to_business_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "abusive_reports_user_to_business"), $this->session->userdata('iAdminId'));
                $data_arr = $this->abusive_reports_user_to_business_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "abusive_reports_user_to_business", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "abusive_reports_user_to_business"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "abusive_reports_user_to_business");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->abusive_reports_user_to_business_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->abusive_reports_user_to_business_model, $phpfunc))
                            {
                                $tmpdata = $this->abusive_reports_user_to_business_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->abusive_reports_user_to_business_model, $fd_callback))
                                {
                                    $fd_status = $this->abusive_reports_user_to_business_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("abusive_reports_user_to_business_add");
                    }
                    else
                    {
                        $this->loadView("abusive_reports_user_to_business_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("abusive_reports_user_to_business_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("abusive_reports_user_to_business_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            if ($mode == 'Add')
            {
                $id = $this->abusive_reports_user_to_business_model->insert($data);
                if (intval($id) > 0)
                {
                ...
                $switch_combo = $this->abusive_reports_user_to_business_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "abusive_reports_user_to_business", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->abusive_reports_user_to_business_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iAbusiveReportsId"] = $data["iAbusiveReportsId"] = $id;
                ...
                $switch_combo = $this->abusive_reports_user_to_business_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "abusive_reports_user_to_business", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->abusive_reports_user_to_business_model->getListConfiguration();
            $config_arr['form_config'] = $this->abusive_reports_user_to_business_model->getFormConfiguration();
            $config_arr['table_name'] = $this->abusive_reports_user_to_business_model->table_name;
            $config_arr['table_alias'] = $this->abusive_reports_user_to_business_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->abusive_reports_user_to_business_model->table_alias.".".$this->abusive_reports_user_to_business_model->primary_key;
        }
        else
        {
            $primary_field = $this->abusive_reports_user_to_business_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("abusive_reports_user_to_business_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->abusive_reports_user_to_business_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("abusive_reports_user_to_business_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("abusive_reports_user_to_business", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->abusive_reports_user_to_business_model->getListConfiguration($post_name);
                    $form_config = $this->abusive_reports_user_to_business_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->abusive_reports_user_to_business_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->abusive_reports_user_to_business_model->table_alias.".";
                    }
                    else
                    {
                    ...
                    $success = $this->abusive_reports_user_to_business_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->abusive_reports_user_to_business_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->abusive_reports_user_to_business_model, $phpfunc))
                    {
                        $data_arr = $this->abusive_reports_user_to_business_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->abusive_reports_user_to_business_model->switchto_fields;
        $extra_cond = $this->abusive_reports_user_to_business_model->extra_cond;
        ...
        $switch_arr = $this->abusive_reports_user_to_business_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->abusive_reports_user_to_business_model->getListConfiguration($alias_name);
