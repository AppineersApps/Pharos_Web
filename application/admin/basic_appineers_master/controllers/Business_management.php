 ...
 *
 * @class Business_management.php
 *
 * @path application\admin\basic_appineers_master\controllers\Business_management.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Business_management extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('business_management_model');
        $this->folder_name = "basic_appineers_master";
        $this->module_name = "business_management";
        ...
        $this->dropdown_arr = array(
            ...
            "u_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_INACTIVE')
                    ),
                    array(
                        'id' => 'Pending',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_PENDING')
                    ),
                    array(
                        'id' => 'Archived',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_ARCHIVED')
                    )
                )
            ),
            "u_push_notify" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Yes',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_YES')
                    ),
                    array(
                        'id' => 'No',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_NO')
                    )
                )
            ),
            ...
            "u_social_login_type" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'facebook',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_FACEBOOK')
                    ),
                    array(
                        'id' => 'g+',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_G_C43')
                    )
                )
            ),
            "u_device_type" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'ios',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_IOS')
                    ),
                    array(
                        'id' => 'android',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_ANDROID')
                    )
                )
            ),
            ...
            "u_log_status_updated" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_LOG_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('BUSINESS_MANAGEMENT_LOG_INACTIVE')
                    )
                )
            ),
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "business_management_list",
                    "business_management_view",
                    "business_management_add",
                    "business_management_update",
                    "business_management_delete",
                    "business_management_export",
                    "business_management_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("business_management", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "business_management");
            ...
            $status_label = array(
                'js_lang_label.BUSINESS_MANAGEMENT_ACTIVE',
                'js_lang_label.BUSINESS_MANAGEMENT_INACTIVE',
                'js_lang_label.BUSINESS_MANAGEMENT_PENDING',
                'js_lang_label.BUSINESS_MANAGEMENT_ARCHIVED',
            );

            $list_config = $this->business_management_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "business_management");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->business_management_model->default_filters,
            );
            ...
            {
                $this->loadView("business_management_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->business_management_model->getListConfiguration();
        $form_config = $this->business_management_model->getFormConfiguration();
        $extra_cond = $this->business_management_model->extra_cond;
        $groupby_cond = $this->business_management_model->groupby_cond;
        $having_cond = $this->business_management_model->having_cond;
        $orderby_cond = $this->business_management_model->orderby_cond;
        ...
        $data_recs = $this->business_management_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_BUSINESS_MANAGEMENT_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("business_management_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("business_management", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->business_management_model->getListConfiguration();
        $form_config = $this->business_management_model->getFormConfiguration();
        $table_name = $this->business_management_model->table_name;
        $table_alias = $this->business_management_modeltable_alias;
        $primary_key = $this->business_management_model->primary_key;
        $extra_cond = $this->business_management_model->extra_cond;
        $groupby_cond = $this->business_management_model->groupby_cond;
        $having_cond = $this->business_management_model->having_cond;
        $orderby_cond = $this->business_management_model->orderby_cond;
        ...
        $db_recs = $this->business_management_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('BUSINESS_MANAGEMENT_BUSINESS_MANAGEMENT');
        $misc_info['filename'] = "business_management_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("business_management_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("business_management", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("business_management", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "business_management"), $this->session->userdata('iAdminId'));

                $hours_arr = $this->business_management_model->getBussinessHours(intval($id));

                $data_arr = $this->business_management_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "business_management", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "business_management"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "business_management");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->business_management_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->business_management_model, $phpfunc))
                            {
                                $tmpdata = $this->business_management_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->business_management_model, $fd_callback))
                                {
                                    $fd_status = $this->business_management_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("business_management_add");
                    }
                    else
                    {
                        $this->loadView("business_management_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("business_management_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("business_management_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("business_management", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            //$b_about_business = $params_arr["b_about_business"];
            $u_one_time_transaction = $params_arr["u_one_time_transaction"];
            ...
            //$data["tAboutBusiness"] = $b_about_business;
            ...
            //$save_data_arr["b_about_business"] = $data["tAboutBusiness"];
            if ($mode == 'Add')
            {
                $id = $this->business_management_model->insert($data);
                if (intval($id) > 0)
                {
                ...
                $switch_combo = $this->business_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "business_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->business_management_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iUserId"] = $data["iUserId"] = $id;
                ...
                $switch_combo = $this->business_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "business_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->business_management_model->getListConfiguration();
            $config_arr['form_config'] = $this->business_management_model->getFormConfiguration();
            $config_arr['table_name'] = $this->business_management_model->table_name;
            $config_arr['table_alias'] = $this->business_management_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->business_management_model->table_alias.".".$this->business_management_model->primary_key;
        }
        else
        {
            $primary_field = $this->business_management_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("business_management_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("business_management", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $main_data = $this->business_management_model->getData($main_cond, array('u.vProfileImage'));
                    $success = $this->business_management_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("business_management_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("business_management", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->business_management_model->getListConfiguration($post_name);
                    $form_config = $this->business_management_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->business_management_model->getData(intval($primary_ids));

                    $data_arr[$field_name] = $post_val;
                    $success = $this->business_management_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->business_management_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->business_management_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->business_management_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->business_management_model, $phpfunc))
                    {
                        $data_arr = $this->business_management_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->business_management_model->switchto_fields;
        $extra_cond = $this->business_management_model->extra_cond;
        ...
        $switch_arr = $this->business_management_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->business_management_model->getListConfiguration($alias_name);
    ...
    public function downloadListFile()
    {
        ...
        if (is_array($config_arr) && count($config_arr) > 0)
        {
            ...
            $data_arr = $this->business_management_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->business_management_model->getFormConfiguration($unique_name);
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->business_management_model->update($update_arr, intval($id));
                if (!$res)
                {
